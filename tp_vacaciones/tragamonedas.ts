...
export default class Tragamonedas {
    private juegoPrendido: boolean;
    public constructor(juegoPrendido: boolean) {
        this.juegoPrendido = juegoPrendido;
    }
    public getJuegoPrendido(): void {
        if (this.juegoPrendido) {
            this.juegoPrendido = false;
        } else {
            this.juegoPrendido = true;
        };
    };
    public getResultadoNumero(): void {
        ...
        while (monto != 0) {
            ...
            if ((carrete1 == carrete2) && (carrete1 == carrete3)) {
                ...
                FS.appendFileSync("casinoRecaudacion.txt", `En este tragamonedas, hemos recaudado un total de :  ${recaudacion} pesos\n`);
        ...
        FS.appendFileSync("tragamonedas.txt", `En total hubo ${premioTotal} pesos en premios\n`);
    };
    ...
    public getResultadoFruta(): void {
        ...
        while (monto != 0) {
            ...
            FS.appendFileSync("casinoRecaudacion.txt", `En este tragamonedas especial, hemos recaudado un total de : ${recaudacion} pesos\n`);
            monto = RLS.questionInt("Si desea seguir jugando ingrese el monto de su apuesta, sino presione 0 para finalizar: ");
        };
        FS.writeFileSync("tragamonedas.txt", `Hubo un total de ${premioTotal} pesos en premios\n`);
    };
